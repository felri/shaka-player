...
shaka.media.PresentationTimeline = class {
   ...
   * @param {number=} maxSegmentDuration The maximum segment duration
   *   (in seconds).
   *
  ...
  constructor(presentationStartTime, presentationDelay,
      autoCorrectDrift = true, maxSegmentDuration = 1) {
    /** @private {?number} */
  ...
  /**
   * Advances the minimum known segment start time.
   * Used when old segments are evicted from the index to shrink the seek range.
   *
   * @param {number} startTime
   */
  advanceMinSegmentStartTime(startTime) {
    if (this.minSegmentStartTime_ == null ||
        startTime > this.minSegmentStartTime_) {
      this.minSegmentStartTime_ = startTime;
    }
  }
