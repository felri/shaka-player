...
shaka.hls.HlsParser = class {
  ...
  constructor() {
    ...
    /**
     * Tracks already-processed DATERANGE IDs to avoid redundant work on each
     * live playlist update. Only completed date ranges (with known endTime)
     * are cached here.
     * @private {!Set<string>}
     */
    this.seenDateRangeIds_ = new Set();
  ...
  async updateStream_(streamInfo) {
    ...
    const maxLiveSegments = this.config_.hls.maxLiveSegments;
    let data = response.data;
    if (maxLiveSegments < Infinity &&
        this.presentationTimeline_.isDynamic()) {
      const text = shaka.util.StringUtils.fromUTF8(data);
      if (!text.includes('#EXT-X-ENDLIST') &&
          !text.includes('SKIPPED-SEGMENTS')) {
        data = shaka.util.StringUtils.toUTF8(
            this.truncatePlaylistText_(text, maxLiveSegments));
      }
    }
  ...
  async processDateRangeTags_(tags, contentType, variables, getUris) {
    ...
    for (let i = 0; i < dateRangeTags.length; i++) {
      ...
      try {
        ...
        // Skip already-processed completed date ranges across updates.
        const dateRangeId = tag.getAttributeValue('ID');
        if (dateRangeId && endTime != null &&
            this.seenDateRangeIds_.has(dateRangeId)) {
          continue;
        }
        ...
        if (dateRangeId && endTime != null) {
          this.seenDateRangeIds_.add(dateRangeId);
        }
      } catch (e) {
  ...
  /**
   * Truncates a live media playlist text to keep only the last N segments,
   * preserving playlist-level tags, carry-forward tags (EXT-X-MAP, KEY, etc.),
   * and trailing tags (RENDITION-REPORT, PRELOAD-HINT). Updates
   * EXT-X-MEDIA-SEQUENCE to reflect the skipped segments.
   *
   * @param {string} text Full playlist text
   * @param {number} maxSegments Maximum segments to keep
   * @return {string} Truncated playlist text
   * @private
   */
  truncatePlaylistText_(text, maxSegments) {
    /**
     * Strip the base64 payload from X-ASSET-LIST referenced attributes
     * to reduce text size for the parser.
     * @param {string} line
     * @return {string}
     */
    const stripDateRangePayload_ = (line) => {
      const match = line.match(/X-ASSET-LIST="([^"]+)"/);
      if (match) {
        const attrName = match[1];
        const escaped = attrName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        line = line.replace(new RegExp(escaped + '="[^"]*"', 'g'),
            attrName + '=""');
      }
      return line;
    };
    ...
    let segCount = 0;
    let cutoff = totalLines;
    for (let i = totalLines - 1; i >= headerEnd; i--) {
      if (lines[i].startsWith('#EXTINF:')) {
        segCount++;
    ...
    if (cutoff >= totalLines) {
      return text;
    }

    const header = lines.slice(0, headerEnd);
    const carry = [];
    const keepSeg = new Set(['EXT-X-MAP', 'EXT-X-KEY',
      'EXT-X-PROGRAM-DATE-TIME']);
    let skippedMediaSequence = 0;
    for (let i = headerEnd; i < cutoff; i++) {
      const line = lines[i];
      if (line.startsWith('#EXTINF:')) {
        skippedMediaSequence++;
      }
      for (const name of keepSeg) {
        if (line.startsWith('#EXT-' + name + ':')) {
          carry.push(line);
          break;
        }
      }
    ...
    for (let i = 0; i < header.length; i++) {
      if (header[i].startsWith('#EXT-X-MEDIA-SEQUENCE:')) {
        const currentSeq = parseInt(header[i].split(':')[1], 10) || 0;
        header[i] = '#EXT-X-MEDIA-SEQUENCE:' +
            (currentSeq + skippedMediaSequence);
        break;
      }
    }

    const trailing = [];
    for (let i = cutoff; i < totalLines; i++) {
      const line = lines[i];
    ...
    for (let i = cutoff; i < totalLines; i++) {
      ...
      if (!line.startsWith('#EXT-X-RENDITION-REPORT:') &&
        ...
        if (line.startsWith('#EXT-X-DATERANGE:')) {
          line = stripDateRangePayload_(line);
        }
        body.push(line);
      }
    }

    return [...header, ...carry, ...body, ...trailing].join('\n');
  }
};
