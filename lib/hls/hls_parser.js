  ...
  async updateStream_(streamInfo) {
    ...
    if (totalCount) {
      ...
      if (maxLiveSegments < Infinity) {
        const beforeCount = stream.segmentIndex.getNumReferences();
        while (stream.segmentIndex.getNumReferences() > maxLiveSegments) {
          const earliest = stream.segmentIndex.earliestReference();
          if (earliest) {
            stream.segmentIndex.evict(earliest.endTime);
          } else {
            break;
          }
        }
        if (stream.segmentIndex.getNumReferences() < beforeCount) {
          const newEarliest = stream.segmentIndex.earliestReference();
          if (newEarliest) {
            this.presentationTimeline_.advanceMinSegmentStartTime(
                newEarliest.startTime);
          }
        }
      }
    }
