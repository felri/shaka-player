...
shaka.media.StreamingEngine = class {
  ...
  createMediaState_(stream) {
    ...
    const mediaState = {
      ...
      needsSegmentIndexRefresh: false,
    };
    if (stream.dependencyStream) {
      mediaState.dependencyMediaState =
          this.createMediaState_(stream.dependencyStream);
  ...
  async onUpdate_(mediaState) {
    ...
    // If the segment index is stale (e.g. for live streams after interstitials
    // or long pauses), close and recreate it.
    if (mediaState.needsSegmentIndexRefresh &&
        this.manifest_.presentationTimeline.isDynamic()) {
      mediaState.needsSegmentIndexRefresh = false;
      if (mediaState.stream.closeSegmentIndex) {
        mediaState.stream.closeSegmentIndex();
      }
      mediaState.segmentIterator = null;
  ...
  update_(mediaState) {
    ...
    if (!reference) {
      ...
      if (this.manifest_.presentationTimeline.isDynamic()) {
        mediaState.needsSegmentIndexRefresh = true;
      }
      let lastSegmentDuration = Infinity;
      const prevSegmentReference = mediaState.lastSegmentReference;
      if (prevSegmentReference) {
        lastSegmentDuration =
            prevSegmentReference.endTime - prevSegmentReference.startTime;
      }
 ...
 *   needsSegmentIndexRefresh: boolean,
 * }}
 *
 * @description
 ...
 * @property {boolean} needsSegmentIndexRefresh
 *   True indicates that the segment index needs to be refreshed (e.g. for live
 *   streams after interstitials or long pauses).
 */
shaka.media.StreamingEngine.MediaState_;
